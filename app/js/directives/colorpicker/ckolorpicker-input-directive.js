        ...
        return {
            ...
            scope: {
                ...
                defaultColor: '=',  // OPTIONAL: If the model is an invalid color string, use this instead
                				onblur: '=',   		// OPTIONAL: Enable or disable the css blur filter behind the overlay. Enabled by default

				onselect: '&onselect'   		// OPTIONAL: Enable or disable the css blur filter behind the overlay. Enabled by default
            },
            templateUrl: 'templates/directives/ackolor.html',
            link: function(scope,elm){
                ...
                var wheelZ = body.querySelector('.zone');
                var $wheelZ = angular.element(wheelZ);
                /* End DOM Definitions */
                ...
                scope.toggleCKoloring = function(){
					var existing = wheelZ.querySelector('a-ckolor-wheel');
					if(!existing){
						var template = '<a-ckolor-wheel></a-ckolor-wheel>';
						var content = $compile(template)(scope);

						$wheelZ.append(content);
						wheel = wheelZ.querySelector('.c-ckolor__wheel');
					}
                    ...
                    if(newVal !== oldVal && CKolorFactory.modelId === modelId && newVal !== scope.model){
                        ...
                        switch(CKolorFactory.originalFormat){
                            case 'hex':
                                ...
                                if(valid){ 
                                    scope.model = CKolorFactory.model;
                                                              var char = scope.onselect();

                                                    char({colors : CKolorFactory.model});
                             }
