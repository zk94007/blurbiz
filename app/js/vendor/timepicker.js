...
.controller('syTimepickerController', ['$scope', '$attrs', '$parse', '$log', '$locale', 'syTimepickerConfig', 
  ...
  $scope.getUserTimezone = function() {
    var gc_array = _.values(timezone_from_google_calendar);
    for ( var index in gc_array)
    {
      if (gc_array[index][moment().tz()])
      {
        return gc_array[index][moment().tz()].replace(/\(.+\)/, '').trim();
      }
    }
    return null;
  };
